...
def execute(sql, *args):
    with pymysql.connect(host='localhost', user='root', password='123456', db='douban_comment', port=3306,
                         charset='utf8') as cur:
        cur.execute(sql, *args)


def executemany(sql, args):
    conn = pymysql.connect(host='localhost', user='root', password='123456', db='douban_comment', port=3306,
                           charset='utf8')
    cur = conn.cursor()
...
for url in url_list:
    ...
    for index, item in enumerate(comments):
        ...
        #        print temp
        execute("insert into content (date,user,comment,vote) values(%s,%s,%s,%s) ", temp)
        ...
        # for index, item in enumerate(comments):
        #     a = time[index].get_text().strip()
        #     b = users[index].get_text().strip()
        #     c = comments[index].get_text().strip()
        #     d = count[index].get_text().strip()
        #     execute('''insert into content (date,user,comment,vote) values(%s,%s,%s,%s) ''' % (a,b,c,d))
